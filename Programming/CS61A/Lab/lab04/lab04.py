    ...
    def add(fruits, amount, cart):
        if fruits == [] and amount == 0:
            ...
            for k in range(amount // price):
                ...
                add([fruit_to_cart for fruit_to_cart in fruits if fruit_to_cart != fruit], amount - (k + 1) * price, cart + display(fruit, k + 1))
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    city_v = make_city('_', lat, lon)
    return get_name(city_a) if distance(city_a, city_v) < distance(city_b, city_v) else get_name(city_b)
