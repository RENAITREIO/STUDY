...
bool RobinHoodHashTable::insert(const string& elem) {
    if (contains(elem) || numElements == numSlots) {
        return false;
    }

    Slot tmp = {elem, 0};
    int index = function(elem) % numSlots;
    while (true) {
        if (elems[index].distance < 0) {
            elems[index] = tmp;
            break;
        }
        else if (tmp.distance > elems[index].distance) {
            swap(tmp, elems[index]);
        }
...
bool RobinHoodHashTable::contains(const string& elem) const {
    ...
    int distance = 0;
    do {
        if (distance > elems[i].distance) {
            return false;
        }
        if (elems[i].distance >= 0 && elems[i].value == elem) {
            return true;
        }
        distance++;
        i = (i + 1) % numSlots;
...
bool RobinHoodHashTable::remove(const string& elem) {
    ...
    int index = function(elem) % numSlots;
    while (elems[index].value != elem || elems[index].distance < 0) {
        index = (index + 1) % numSlots;
    }
    elems[index].distance = EMPTY_SLOT;

    while (elems[(index + 1) % numSlots].distance > 0) {
        swap(elems[index], elems[(index + 1) % numSlots]);
