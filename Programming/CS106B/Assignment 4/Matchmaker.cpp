...
void maxWeightMatchingHelper(const Map<string, Map<string, int>>& possibleLinks, const Map<string, Map<string, int>>& copy, Set<Pair>& tmp, Set<Pair>& ans);
int computeWeight(const Map<string, Map<string, int>>& possibleLinks, Set<Pair>& pairs);
...
bool hasPerfectMatching(const Map<string, Set<string>>& possibleLinks, Set<Pair>& matching) {
    ...
    Set<string> pairs = possibleLinks.get(first);
    for (string person: pairs) {
        if (!possibleLinks.containsKey(person)) {
            continue;
        }
        // chosen
        Pair chosen(first, person);
        matching.add(chosen);

        Map<string, Set<string>> links = possibleLinks;
        links.remove(first);
        links.remove(person);
        // explore
        bool res = hasPerfectMatching(links, matching);
        if (res == true) {
            return res;
        }
        // unchosen
        matching.remove(chosen);
    }
    return false;
}
...
Set<Pair> maximumWeightMatching(const Map<string, Map<string, int>>& possibleLinks) {
    Set<Pair> ans;
    Set<Pair> tmp;
    maxWeightMatchingHelper(possibleLinks, possibleLinks, tmp, ans);
    return ans;
}

void maxWeightMatchingHelper(const Map<string, Map<string, int>>& possibleLinks,
                             const Map<string, Map<string, int>>& copyForCompute,
                             Set<Pair>& tmp,
                             Set<Pair>& ans) {
    if (possibleLinks.isEmpty() || (possibleLinks.size() == 1 && !tmp.isEmpty())) {
        if (computeWeight(copyForCompute, tmp) > computeWeight(copyForCompute, ans)) {
            ans = tmp;
        }
        return;
    }
    ...
    string first = possibleLinks.firstKey();
    Map<string, Map<string, int>> links = possibleLinks;
    links.remove(first);
    for (string person: possibleLinks.get(first)) {
        if (!possibleLinks.containsKey(person)) {
            continue;
        }
        if (possibleLinks.get(first).get(person) >= 0) {
            // chosen
            Pair chosen(first, person);

            Map<string, int> addBack = links[person];
            tmp.add(chosen);
            links.remove(person);
            // explore
            maxWeightMatchingHelper(links, copyForCompute, tmp, ans);
            // unchosen
            tmp.remove(chosen);
            links.put(person, addBack);
        }
    }
    maxWeightMatchingHelper(links, copyForCompute, tmp, ans);
}
...
int computeWeight(const Map<string, Map<string, int>>& possibleLinks, Set<Pair>& pairs) {
    ...
    int sum = 0;
    for (Pair pair: pairs) {
        sum += possibleLinks.get(pair.first()).get(pair.second());
    }
    return sum;
}
...
PROVIDED_TEST("Test hasPerfectMatching.") {
    auto links = fromLinks({
        { "A", "B" },
        { "B", "C" },
        { "C", "D" },
...
PROVIDED_TEST("Test.") {
    ...
    EXPECT_EQUAL(maximumWeightMatching(links), { {"B", "E"}, {"C", "D"} });
}
