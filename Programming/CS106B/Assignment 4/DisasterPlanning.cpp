...
bool planHelper(const Map<string, Set<string>>& roadNetwork,
                int numCities,
                Set<string>& supplyLocations,
                Set<string>& notCoverCities);

bool canBeMadeDisasterReady(const Map<string, Set<string>>& roadNetwork,
                            int numCities,
                            Set<string>& supplyLocations) {
    if (numCities < 0) {
        error("numCities is negative.");
    }
    Set<string> notCoverCities;
    for (string city: roadNetwork) {
        notCoverCities.add(city);
    }
    return planHelper(roadNetwork, numCities, supplyLocations, notCoverCities);
}
...
bool planHelper(const Map<string, Set<string>>& roadNetwork,
    ...
    Set<string> linkedCities = roadNetwork[notCoverCities.first()];
    linkedCities.add(notCoverCities.first());
    for (string city: linkedCities) {
        Set<string> tmp = notCoverCities;
        // chosen
        supplyLocations.add(city);
        tmp.remove(city);
        tmp -= roadNetwork[city];
        // explore
        if (planHelper(roadNetwork, numCities - 1, supplyLocations, tmp) == true) {
            return true;
        }
        // unchosen
