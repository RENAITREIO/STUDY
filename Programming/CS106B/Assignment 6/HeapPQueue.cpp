...
void swapData(DataPoint& data1, DataPoint& data2) {
    DataPoint tmp = data1;
    data1 = data2;
    data2 = tmp;
}
...
void HeapPQueue::enqueue(const DataPoint& data) {
    if (logicalSize == allocatedSize - 1) {
        allocatedSize *= 2;
        DataPoint *ptr = new DataPoint[allocatedSize];
    ...
    int tmp = logicalSize;
    while (tmp > 1 && elems[tmp].weight < elems[tmp / 2].weight) {
        swapData(elems[tmp], elems[tmp / 2]);
        tmp /= 2;
    }
}
...
DataPoint HeapPQueue::dequeue() {
    ...
    logicalSize--;
    int index = 1;
    while (index * 2 <= logicalSize) {
        if (elems[index].weight > elems[index * 2].weight) {
            if (index * 2 + 1 <= logicalSize && elems[index * 2].weight > elems[index * 2 + 1].weight) {
                swapData(elems[index * 2 + 1], elems[index]);
                index = index * 2 + 1;
            }
            else {
                swapData(elems[index], elems[index * 2]);
                index = index * 2;
            }
        }
        else if (index * 2 + 1 <= logicalSize && elems[index].weight > elems[index * 2 + 1].weight) {
            swapData(elems[index * 2 + 1], elems[index]);
            index = index * 2 + 1;
        }
        else {
