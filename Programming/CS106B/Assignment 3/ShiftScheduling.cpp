...
void addHelper(Set<Shift>& shifts, Set<Shift>& ans, Set<Shift>& tmps, int hours);
bool canAdd(Shift& tmp, Set<Shift>& tmps, int hours);
int totalValue(Set<Shift>& shifts);
...
Set<Shift> highestValueScheduleFor(const Set<Shift>& shifts, int maxHours) {
    ...
    Set<Shift> tmpShifts = shifts;
    Set<Shift> undo;
    addHelper(tmpShifts, ans, undo, maxHours);
    return ans;
}

// recursive function
void addHelper(Set<Shift>& shifts, Set<Shift>& ans, Set<Shift>& tmps, int hours) {
    if (shifts.isEmpty() || hours <= 0) {
        if (totalValue(ans) < totalValue(tmps)) {
            ans = tmps;
        }
        return;
    }

    Shift tmp = shifts.first();

    // chosen
    shifts.remove(tmp);
    // explore
    addHelper(shifts, ans, tmps, hours);
    // unchosen
    shifts.add(tmp);

    if (canAdd(tmp, tmps, hours)) {
        // chosen
        tmps.add(tmp);
        shifts.remove(tmp);
        // explore
        addHelper(shifts, ans, tmps, hours - lengthOf(tmp));
        // unchosen
        tmps.remove(tmp);
        shifts.add(tmp);
    }
...
bool canAdd(Shift& tmp, Set<Shift>& tmps, int hours) {
    if (lengthOf(tmp) > hours) {
        return false;
    }
    for (Shift shift: tmps) {
        if (overlapsWith(shift, tmp)) {
            return false;
        }
    }
    return true;
...
PROVIDED_TEST("Test.") {
    ...
    EXPECT_EQUAL(highestValueScheduleFor(shifts, 24).size(), 0);
    EXPECT_EQUAL(highestValueScheduleFor(shifts, 24), shifts);
    EXPECT_EQUAL(highestValueScheduleFor(shifts, 5).size(), 0);
}
