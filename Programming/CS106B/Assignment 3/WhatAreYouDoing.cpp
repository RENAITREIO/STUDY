...
void allEmphasesHelper(Vector<string> words, Set<string>& ans, string tmp = "");

Set<string> allEmphasesOf(const string& sentence) {
    Vector<string> words = tokenize(sentence);
    Set<string> ans;
    allEmphasesHelper(words, ans);
    return ans;
}
...
void allEmphasesHelper(Vector<string> words, Set<string>& ans, string tmp) {
    ...
    allEmphasesHelper(words.subList(index + 1), ans, tmp + toLowerCase(words[index]));
    allEmphasesHelper(words.subList(index + 1), ans, tmp + toUpperCase(words[index]));
}
