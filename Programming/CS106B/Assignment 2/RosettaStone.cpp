...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
    ...
    int loopTimes = str.length() - kGramLength + 1;
...
Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
    ...
    PriorityQueue<string> filter;
    for (string str: source) {
        filter.enqueue(str, source[str]);
    }
    int length = source.size();
    int index = length - numToKeep;
    for (int i = 0; i < length; i++) {
        if (i < index) {
            filter.dequeue();
        }
        else {
            string tmp = filter.dequeue();
            res.put(tmp, source[tmp]);
        }
    }
    return res;
}
...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    for (Corpus cor: corpora) {
        double tmp = cosineSimilarityOf(textProfile, cor.profile);
        if (max < tmp) {
            str = cor.name;
            max = tmp;
        }
