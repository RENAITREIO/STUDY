...
public class GraphDB {
    ...
    long closest(double lon, double lat) {
        ...
        for (long v: nodes.keySet()) {
            if (closestId == -1) {
                ...
                minDistance = distance(lon, lat, node.lon, node.lat);
                continue;
            }
            Node node = nodes.get(v);
    ...
    public class Node {
        ...
        private double lon;
        private double lat;
        private final Set<Long> adj;
        // name is null if the node is not a location
        ...
        public void addAdjNode(Long node) {
            adj.add(node);
        }
