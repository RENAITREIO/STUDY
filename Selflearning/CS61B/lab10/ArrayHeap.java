...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void sink(int index) {
        ...
        int minIdx = min(leftIndex(index), rightIndex(index));
        while (index > 0 && contents[minIdx] != null
                && contents[index].priority() > contents[minIdx].priority()) {
            swap(index, minIdx);
            index = minIdx;
            minIdx = min(leftIndex(index), rightIndex(index));
            if (minIdx > size) {
                break;
            }
        }
