...
Nucleotide* findFirst(Nucleotide* dna, Nucleotide* target) {
    ...
    Nucleotide* res = nullptr;
    while (dna != nullptr) {
        Nucleotide* targetCpy = target;
        Nucleotide* dnaCpy = dna;
        bool foundFlag = true;
        while (targetCpy != nullptr) {
            if (targetCpy->value != dnaCpy->value ||
                (dnaCpy->next == nullptr && targetCpy->next != nullptr)) {
                foundFlag = false;
                break;
            }
            targetCpy = targetCpy->next;
            dnaCpy = dnaCpy->next;
        }
        if (foundFlag == true) {
            res = dna;
...
void deleteNode(Nucleotide*& target, Nucleotide*& targetDNA) {
    while (target != nullptr) {
        Nucleotide* toDelete = targetDNA;
        targetDNA = targetDNA->next;
        target = target->next;
        delete toDelete;
    }
}
...
bool spliceFirst(Nucleotide*& dna, Nucleotide* target) {
    ...
    Nucleotide* targetDNA = findFirst(dna, target);
    if (targetDNA == nullptr) {
        return false;
    }

    if (targetDNA == dna) {
        deleteNode(target, targetDNA);
        dna = targetDNA;
        if (dna != nullptr) {
            dna->prev = nullptr;
        }
        ...
        Nucleotide* ptr = targetDNA;
        targetDNA = targetDNA->prev;
        deleteNode(target, ptr);
        targetDNA->next = ptr;
        if (ptr != nullptr) {
            ptr->prev = targetDNA;
        }
...
PROVIDED_TEST("Test deleteNucleotides.") {
    ...
    dna->next->next->value = 'C';
    dna->next->next->prev = dna;
...
PROVIDED_TEST("Test fromDNA.") {
    ...
    EXPECT_EQUAL(fromDNA(dna), "ABC");

    deleteNucleotides(dna);
}


PROVIDED_TEST("Test toStrand.") {
    string original = "null-A-C-G-T-null";
...
PROVIDED_TEST("Test spliceFirst.") {
    Nucleotide* dna    = toStrand("ACCCGGGCCCGGG");
    ...
    EXPECT_EQUAL(fromDNA(dna), "ACCCGGG");
