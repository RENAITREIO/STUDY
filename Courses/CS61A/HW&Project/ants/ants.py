...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        locate_num = 0
        while local_place.is_hive != True and locate_num <= self.upper_bound:
            bee = random_bee(local_place.bees)
            if bee != None and locate_num >= self.lower_bound:
                return bee
            else:
                local_place = local_place.entrance
            locate_num += 1
        return None
        # END Problem 3 and 4
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        "*** YOUR CODE HERE ***"
        super().action(gamestate)
        local_place = self.place.exit
        while local_place != None:
            if local_place.ant != None:
                local_place.ant.double()
                if local_place.ant.is_container == True and local_place.ant.ant_contained != None:
                    local_place.ant.ant_contained.double()
            local_place = local_place.exit
        # END Problem 12
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        if not hasattr(target, 'old_action_slow'):
            target.old_action_slow = target.action
            def slowed_action(gamestate):
                if target.slow_count <= 0 or gamestate.time % 2 == 0:
                    target.old_action_slow(gamestate)
                target.slow_count -= 1
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        distance = 0
        local_place = self.place
        while local_place.is_hive == False:
            for bee in local_place.bees:
                distance_dict[bee] = distance
            if distance == 0:
                if self.place.ant.is_container:
                    distance_dict[self.place.ant] = distance
            else:
                if local_place.ant != None:
                    distance_dict[local_place.ant] = distance
            local_place, distance = local_place.entrance, distance + 1
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        gamestate -- The GameState, used to access game state information.
        """
        if hasattr(self, 'back_length') and self.back_length > 0:
            self.action_back(gamestate)
            self.back_length -= 1
        else:
    ...
    def scare(self, length):
        ...
        # BEGIN Problem EC 2
        "*** YOUR CODE HERE ***"
        if not hasattr(self, 'back_length'):
            self.back_length = length
            def action_back(gamestate):
                destination = self.place.entrance
                if self.blocked():
                    self.sting(self.place.ant)
                elif self.health > 0 and destination.is_hive == False:
                    self.move_to(destination)
