...
def accuracy(typed, source):
    ...
    true_count, i = 0, 0
    while i < min(typed_length, source_length):
        if typed_words[i] == source_words[i]:
            true_count += 1
        i += 1
    return true_count / typed_length * 100.0
    # END PROBLEM 3
...
def memo_diff(diff_function):
    ...
    def memoized(typed, source, limit):
        # BEGIN PROBLEM EC
        "*** YOUR CODE HERE ***"
        key_tuple = (typed, source)
        if key_tuple in cache and cache[key_tuple][1] >= limit:
            return cache[key_tuple][0]
        else:
            value = diff_function(typed, source, limit)
            cache[key_tuple] = (value, limit)
            return value
        # END PROBLEM EC
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    res_word, min = word_list[0], diff_function(typed_word, word_list[0], limit)
    for word in word_list:
        diff = diff_function(typed_word, word, limit)
        if diff < min:
            res_word, min = word, diff
...
def minimum_mewtations(typed, source, limit):
    ...
    if typed[0] == source[0]: # Feel free to remove or add additional cases
        ...
        substitute = typed[1:]
        # BEGIN
        return min(minimum_mewtations(add, source[1:], limit - 1) + 1, minimum_mewtations(remove, source, limit - 1) + 1, minimum_mewtations(substitute, source[1:], limit - 1) + 1)
        # END
...
def report_progress(typed, source, user_id, upload):
    ...
    "*** YOUR CODE HERE ***"
    correct_word, i = 0, 0
    loop_len = min(len(typed), len(source))
    while i < loop_len:
        if typed[i] != source[i]:
            break
        correct_word, i = correct_word + 1, i + 1
    rate = correct_word / len(source)
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    times = [[times_list[i + 1] - times_list[i] for i in range(len(times_list) - 1)] for times_list in timestamps_per_player]
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    for i in word_indices:
        ...
        for index in player_indices:
            if times[fastest_i][i] > times[index][i]:
                fastest_i = index
        res[fastest_i] += [words[i]]
    return res
    # END PROBLEM 10
