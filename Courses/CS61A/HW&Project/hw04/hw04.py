...
def balanced(m):
    ...
    if is_planet(m):
        ...
        left_mobile, right_mobile = end(left_arm), end(right_arm)
        if length(left_arm) * total_mass(left_mobile) == length(right_arm) * total_mass(right_mobile):
            return balanced(left_mobile) and balanced(right_mobile)
        else:
            return False
...
def max_path_sum(t):
    ...
    if is_leaf(t):
        ...
        part_sum = []
        for branch in branches(t):
            part_sum += [max_path_sum(branch)]
        return label(t) + max(part_sum)
