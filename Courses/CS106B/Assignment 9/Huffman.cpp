...
EncodingTreeNode* huffmanTreeFor(const string& str) {
    Map<char, double> chWeight;
    for (char ch: str) {
        if (chWeight.containsKey(ch)) {
            chWeight[ch]++;
        }
        else {
            chWeight.put(ch, 1.0);
...
string decodeText(Queue<Bit>& bits, EncodingTreeNode* tree) {
    ...
    while (!bits.isEmpty()) {
        ...
        while (!(root->zero == nullptr && root->one == nullptr)) {
            if (bits.isEmpty()) {
                error("bits error.");
            }
 ...
 */
void addTable(EncodingTreeNode* tree, Map<char, Queue<Bit>>& table, Queue<Bit>& bits);
...
Queue<Bit> encodeText(const string& str, EncodingTreeNode* tree) {
    ...
    Queue<Bit> forAddTable;
    addTable(tree, table, forAddTable);

    for (char ch: str) {
        Queue<Bit> tmp = table[ch];
        while (!tmp.isEmpty()) {
...
void addTable(EncodingTreeNode* tree, Map<char, Queue<Bit>>& table, Queue<Bit>& bits) {
    ...
    if (tree->zero != nullptr) {
        ...
        bitsZero.enqueue(0);
        addTable(tree->zero, table, bitsZero);
    }

    if (tree->one != nullptr) {
        Queue<Bit> bitsOne = bits;
        bitsOne.enqueue(1);
        addTable(tree->one, table, bitsOne);
    }
}
 ...
 * or bits in them, etc.
 */
void createTree(Queue<Bit>& bits, Queue<char>& leaves, EncodingTreeNode* ptr);

EncodingTreeNode* decodeTree(Queue<Bit>& bits, Queue<char>& leaves) {
    EncodingTreeNode* res = new EncodingTreeNode;
    createTree(bits, leaves, res);
    return res;
}

void createTree(Queue<Bit>& bits, Queue<char>& leaves, EncodingTreeNode* ptr) {
    if (leaves.isEmpty() || bits.isEmpty()) {
        return;
    }

    if (bits.dequeue() == 1) {
        ptr->zero = new EncodingTreeNode;
        createTree(bits, leaves, ptr->zero);

        ptr->one = new EncodingTreeNode;
        createTree(bits, leaves, ptr->one);
    }
    else {
        ptr->ch = leaves.dequeue();
        ptr->zero = nullptr;
        ptr->one = nullptr;
    }
}
...
void encodeTree(EncodingTreeNode* tree, Queue<Bit>& bits, Queue<char>& leaves) {
    ...
    if (ptr->zero == nullptr && ptr->one == nullptr) {
        bits.enqueue(0);
        leaves.enqueue(ptr->ch);
        return;
    }

    bits.enqueue(1);
    encodeTree(ptr->zero, bits, leaves);
    encodeTree(ptr->one, bits, leaves);
}
