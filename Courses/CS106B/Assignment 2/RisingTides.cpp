...
void detectPoint(int point_row, int point_col, const Grid<double>& terrain, double height, Grid<bool>& res, Queue<GridLocation>& points);

Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
                            const Vector<GridLocation>& sources,
                            double height) {
    if (terrain.isEmpty()) {
        error("Terrain is wrong.");
    }
    ...
    Queue<GridLocation> points;
    for (GridLocation point: sources) {
        if (terrain.inBounds(point)) {
            res[point.row][point.col] = true;
            points.enqueue(point);
        }
    }

    while (!points.isEmpty()) {
        GridLocation point = points.dequeue();
        detectPoint(point.row - 1, point.col, terrain, height, res, points);
        detectPoint(point.row, point.col - 1, terrain, height, res, points);
        detectPoint(point.row, point.col + 1, terrain, height, res, points);
        detectPoint(point.row + 1, point.col, terrain, height, res, points);
    }
    return res;
...
void detectPoint(int point_row, int point_col, const Grid<double>& terrain, double height, Grid<bool>& res, Queue<GridLocation>& points) {
    if (terrain.inBounds(point_row, point_col) && terrain[point_row][point_col] <= height && res[point_row][point_col] == false) {
        res[point_row][point_col] = true;
        points.enqueue({point_row, point_col});
    }
}
...
PROVIDED_TEST("Water exist out of bound.") {
    Grid<double> world = {
        { 0, 0, 2 },
        { 0, 2, 0 },
        { 3, 0, 0 }
