...
Map<string, int> apportion(const Map<string, int>& populations, int numSeats) {
    ...
    Map<string, int> res;
    double maxPopulations = 0;
    for (string state: populations) {
        res.put(state, 1);
        if (populations[state] > maxPopulations) {
            maxPopulations = populations[state];
        }
    }
    numSeats -= populations.size();
    maxPopulations /= sqrt(2.0);
    HeapPQueue pq;
    for (string state: populations) {
        pq.enqueue({state, maxPopulations - populations[state] / sqrt(2.0)});
    }

    while (numSeats > 0) {
        DataPoint data = pq.dequeue();
        res[data.name]++;
        numSeats--;
        data.weight = maxPopulations - populations[data.name] / sqrt(res[data.name] * (res[data.name] + 1));
        pq.enqueue(data);
    }
    return res;
}
...
PROVIDED_TEST("Test.") {
    ...
    EXPECT_EQUAL(apportion(populations, 1), seats);
}
