...
Vector<DataPoint> mergeData(const Vector<DataPoint>& data1, const Vector<DataPoint>& data2);
...
Vector<DataPoint> combine(const Vector<Vector<DataPoint>>& sequences) {
    ...
    int halfLength = sequences.size() / 2;
    Vector<DataPoint> res1 = combine(sequences.subList(0, halfLength));
    Vector<DataPoint> res2 = combine(sequences.subList(halfLength));
    return mergeData(res1, res2);
}
